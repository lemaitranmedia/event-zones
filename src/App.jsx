...
const ZONES = [
  { id: "l1_reception",    name: "Tiếp đón L1",       icon: "🏛️", maxCapacity: 8, maxTotal: null },
  { id: "workshop_flower", name: "Workshop cắm hoa",   icon: "🌸", maxCapacity: 9, maxTotal: 15 },
  { id: "workshop_perfume",name: "Workshop nước hoa",  icon: "🌺", maxCapacity: 8, maxTotal: 20 },
  { id: "cinema",          name: "Cinema",             icon: "🎬", maxCapacity: 5, maxTotal: null, hasBusy: true },
  { id: "amenities_l1",   name: "Amenities L1",        icon: "☕", maxCapacity: 6, maxTotal: null },
  { id: "showroom_1br",    name: "Nhà mẫu 1PN",        icon: "🛏️", maxCapacity: 2, maxTotal: null },
  { id: "showroom_2br",    name: "Nhà mẫu 2PN",        icon: "🛏️", maxCapacity: 2, maxTotal: null },
  { id: "showroom_3br",    name: "Nhà mẫu 3PN",        icon: "🛏️", maxCapacity: 2, maxTotal: null },
  { id: "pool",            name: "Hồ bơi",             icon: "🏊", maxCapacity: 4, maxTotal: null },
];

const STAFF_CODES = { "1111": "NV-1111", "2222": "NV-2222", "3333": "NV-3333" };
...
const STATUS = {
  ...
  busy:      { bg: "#f0f0f0", border: "#bbb",    text: "#888",    label: "Đang chiếu" },
};
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, width: 300, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 12, color: "#aaa", marginBottom: 16 }}>Nhập mã nhân viên để xác nhận</div>
...
function StaffView({ zones, onAction }) {
  ...
  const zone = zones.find(z => z.id === zoneId) || { current_count: 0, total_visited: 0, is_busy: false };
  ...
  const maxIn = zoneDef.maxTotal ? Math.min(remaining, zoneDef.maxTotal - zone.total_visited) : remaining;
  ...
  function ask(type) {
    const msgs = {
      ...
      busy: zone.is_busy ? `Kết thúc chiếu phim\ntại "${zoneDef.name}"?` : `Bắt đầu chiếu phim\ntại "${zoneDef.name}"?`,
    };
  ...
  return (
    <div>
      {dialog && <PinDialog title={dialog.msg} onConfirm={handleConfirm} onCancel={() => setDialog(null)} />}
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: `2px solid ${col.border}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#aaa" }}>Tối đa: {zoneDef.maxCapacity}{zoneDef.maxTotal ? ` · Tổng: ${zoneDef.maxTotal} lượt` : ""}</div>
          </div>
        ...
        <div style={{ textAlign: "center", margin: "16px 0" }}>
          ...
          {zoneDef.maxTotal && <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>Đã dùng: {zone.total_visited} / {zoneDef.maxTotal} lượt</div>}
        </div>

        <div style={{ background: "#f0f0f0", borderRadius: 99, height: 10, overflow: "hidden", marginBottom: 18 }}>
          <div style={{ width: `${pct}%`, background: status === "full" ? "#e24b4a" : status === "warning" ? "#fac775" : "#3B6D11", height: "100%", borderRadius: 99, transition: "width 0.3s" }} />
        </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16 }}>
              <button onClick={() => setCount(c => Math.max(1, c - 1))}
                style={{ width: 40, height: 40, borderRadius: "50%", border: "1.5px solid #ddd", background: "#f5f5f5", fontSize: 20, cursor: "pointer", fontWeight: 700 }}>−</button>
              <span style={{ fontSize: 32, fontWeight: 900, color: "#185FA5", minWidth: 40, textAlign: "center" }}>{count}</span>
              <button onClick={() => setCount(c => Math.min(Math.max(maxIn, 1), c + 1))}
                style={{ width: 40, height: 40, borderRadius: "50%", border: "1.5px solid #ddd", background: "#f5f5f5", fontSize: 20, cursor: "pointer", fontWeight: 700 }}>+</button>
            </div>
          </div>
        )}
        ...
        {zoneDef.hasBusy && (
          <button onClick={() => ask("busy")}
            style={{ width: "100%", padding: "11px 0", marginBottom: 12, border: `1px solid ${zone.is_busy ? "#185FA5" : "#ddd"}`, borderRadius: 10, background: zone.is_busy ? "#185FA5" : "#f5f5f5", color: zone.is_busy ? "#fff" : "#555", cursor: "pointer", fontSize: 14, fontWeight: 700 }}>
            {zone.is_busy ? "🎬 Đang chiếu — Bấm để kết thúc" : "▶️ Bắt đầu chiếu phim"}
          </button>
        )}
...
function AdminDashboard({ zones }) {
  ...
  return (
    <div>
      ...
      {ZONES.map(z => {
        const d = zones.find(r => r.id === z.id) || { current_count: 0, total_visited: 0, is_busy: false };
          ...
          <div key={z.id} style={{ background: "#fff", borderRadius: 12, padding: "12px 16px", marginBottom: 8, border: `2px solid ${col.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {z.maxTotal && <span style={{ fontSize: 11, color: "#aaa", background: "#f5f5f5", borderRadius: 10, padding: "2px 8px" }}>{d.total_visited}/{z.maxTotal}</span>}
...
function HistoryTab({ logs }) {
  if (logs.length === 0) return <div style={{ textAlign: "center", padding: 40, color: "#aaa", fontSize: 14 }}>Chưa có lịch sử ghi nhận</div>;
  return (
    <div>
      {logs.map((log, i) => {
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <span style={{ fontWeight: 700, fontSize: 14, color: isIn ? "#3B6D11" : isOut ? "#e24b4a" : "#185FA5" }}>
                  {isIn ? `+${log.count} vào` : isOut ? `−${log.count} ra` : log.action === "busy_on" ? "▶️ Bắt đầu chiếu" : "⏹️ Kết thúc chiếu"}
                </span>
...
export default function App() {
  ...
  useState(() => { load(); const t = setInterval(load, 15000); return () => clearInterval(t); }, []);
  ...
  async function handleAction(zoneId, type, count, staffCode, zoneData) {
    ...
    if (type === "in") {
      ...
      fields = { is_busy: !zoneData.is_busy };
      action = zoneData.is_busy ? "busy_off" : "busy_on";
    }
